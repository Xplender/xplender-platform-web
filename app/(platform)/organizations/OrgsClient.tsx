...
function FilterPills({ active, onChange }: { active: StatusFilter; onChange: (f: StatusFilter) => void }) {
  return (
    <div className="flex items-center gap-1.5 flex-wrap">
      {FILTER_OPTIONS.map((f) => (
        <button
          ...
          className={`px-3 py-1.5 text-xs font-semibold rounded-full border transition-all duration-150 cursor-pointer whitespace-nowrap ${
            active === f.value
              ? "bg-[#4C63FC] text-white border-[#4C63FC]"
              : "bg-white text-[#7B8099] border-[#E2E4EC] hover:border-[#4C63FC] hover:text-[#4C63FC]"
          }`}
        >
          {f.label}
        </button>
    ...
    >
      <div className="flex items-center gap-3 sm:gap-4">
        ...
        <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
          ...
          <span className="text-xs font-semibold text-[#4C63FC] hidden sm:flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
...
export function OrgsClient({ orgs }: { orgs: OrgSummary[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    return orgs.filter((o) => {
      const matchesQ = o.name.toLowerCase().includes(q) || o.slug.toLowerCase().includes(q);
